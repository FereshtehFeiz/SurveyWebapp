...
exports.listAnswers = (surveyId) => {
  return new Promise((resolve, reject) => {
    const sql = "SELECT questionId,answers.userId,answers.surveyId, username, questionTitle, answer FROM answers INNER JOIN questions on questions.id = answers.questionId WHERE answers.surveyId = ?";
    db.all(sql, [surveyId], (err, rows) => {
      if (err) {
        reject(err);
        return;
